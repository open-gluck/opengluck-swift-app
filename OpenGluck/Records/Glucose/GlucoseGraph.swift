...
//            ])
    ...
    var body: some View {
        TimelineView(.everyMinute) { context in
            Impl(
                now: context.date,
                glucoseRecords: filteredGlucoseRecords(forDate: context.date),
...
struct GlucoseGraph_Previews: PreviewProvider {
    struct EmptyPreview: View {
        var body: some View {
            ...
            GlucoseGraph(glucoseRecords: .constant(glucoseRecords), insulinRecords: .constant(insulinRecords), lowRecords: .constant(lowRecords))
        }
    }
    ...
    struct Preview: View {
        ...
        var body: some View {
            ...
            GlucoseGraph(glucoseRecords: .constant(glucoseRecords), insulinRecords: .constant(insulinRecords), lowRecords: .constant(lowRecords), style: style)
                .frame(maxHeight: 200)
        }
    }
